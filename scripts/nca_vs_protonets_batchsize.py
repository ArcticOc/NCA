...
accuracy_ids = [
    "val/1-shot/Best-CL2N",
    "val/5-shot/Best-CL2N",
    "test/1-shot/CL2N",
    "test/5-shot/CL2N",
]
expm_to_color = {
...
for expm in expm_list:
    dataset, batch_size, seed = (
        expm_args[expm]["dataset"],
        expm_args[expm]["batch_size"],
        expm_args[expm]["seed"],
    )
...
for d in datasets:
    for acc in accuracy_ids:
        ...
        plt.legend(
            [expm_to_legend[e] for e in expm_types_plots],
            facecolor="white",
            framealpha=1.0,
            edgecolor="k",
            fancybox=True,
        ...
        plt.savefig(
            os.path.join(
                "../../paper-softnn/art", d + "_" + splits[0] + "_" + splits[1] + ".pdf"
            ),
            bbox_inches="tight",
        )
        # plt.show()
