...
from src.utils.meters import AverageMeter, compute_confidence_interval, warp_tqdm
...
def meta_evaluate(
    data, train_mean, out_cov, shot, num_iter, args, expm_id=None, split=""
):
    cl2n_list = []
    ...
    # save
    if expm_id:
        save_path = os.path.join(args.save_path, "numpy_results")
...
def extract_and_evaluate(
    ...
    shot1_info = tuple(
        [
            100 * x
            for x in meta_evaluate(
                out_dict1, out_mean1, out_cov1, 1, num_iter, args, expm_id, split
            )
        ]
    )
    shot5_info = tuple(
        [
            100 * x
            for x in meta_evaluate(
                out_dict5, out_mean5, out_cov5, 5, num_iter, args, expm_id, split
            )
        ]
    )
